...
public class Physics
{
	...
	/**
	 * Main check for collision of the given shape with something in the
	 * environment.
	 * 
	 * @param s
	 *            : The shape that is being checked against its environment for
	 *            collision.
	 ...
	 * 
	 * @param s1
	 *            : The first shape that is checked for collision
	 * @param s2
	 *            : The second shape that is checked for collision
	 * @return collision status
	 */
	...
	{
		float dis_x, dis_y; // Distance between shapes measured along x and y
							// axis
		double angle, r_s, r_e; // The angle, radius of s, and radius of
								// OBJECTS[i]
		...
		if (Math.sqrt(Math.pow(dis_x, 2) + Math.pow(dis_y, 2)) > r_s + r_e) // Is
																			// the
																			// distance
																			// between
																			// their
	 ...
	 * 
	 * @param s
	 *            : The shape which is being checked for crossing the boundary.
	 ...
	 * 
	 * @param s
	 *            : Shape being deflected by the direction.
	 * @param normal
	 *            : The normal to the plane of deflection.
	 */
	 ...
	 * 
	 * @param m1
	 *            : The first interacting momentum.
	 * @param m2
	 *            : The second interacting momentum.
	 * @param normal
	 *            : The normal of the plane of impact between the momenta.
	 */
	public static void resolveMomentum(Momentum m1, Momentum m2, Direction normal)
	{
		...
		float s_x1 = ((float) (m1.mass - m2.mass) / (m1.mass + m2.mass)) * v1.A() + ((float) (2 * m2.mass) / (m1.mass + m2.mass)) * v2.A();
		float s_x2 = ((float) (2 * m1.mass) / (m1.mass + m2.mass)) * v1.A() + ((float) (m2.mass - m1.mass) / (m1.mass + m2.mass)) * v2.A();

		float s_y1 = ((float) (m1.mass - m2.mass) / (m1.mass + m2.mass)) * v1.B() + ((float) (2 * m2.mass) / (m1.mass + m2.mass)) * v2.B();
		float s_y2 = ((float) (2 * m1.mass) / (m1.mass + m2.mass)) * v1.B() + ((float) (m2.mass - m1.mass) / (m1.mass + m2.mass)) * v2.B();
