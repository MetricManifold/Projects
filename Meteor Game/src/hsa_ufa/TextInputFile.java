...
public class TextInputFile
{
	...
	{
		if (fileName.equalsIgnoreCase("standard input") || fileName.equalsIgnoreCase("keyboard") || fileName.equalsIgnoreCase("stdin"))
		{
	 ...
	 *
	 * @param ch
	 *            - The character to be pushed into the unread position.
	 */
	...
	protected char readACharacter()
	{
		...
		{
			new FatalError("Attempt to read past end of file on " + fileName);
			// Never reaches here.
		}
	 ...
	 *                - Thrown if attempt to read past EOF.
	 * @return The character read from the file.
	 */
		...
		try
		{
			if (useStandardIO)
			{
				...
				if (lineBuffer != null)
				{
					...
					{
						lineBuffer = lineBuffer.substring(0, lineBuffer.indexOf('\032'));
					...
					{
						lineBuffer = lineBuffer.substring(0, lineBuffer.indexOf('\004'));
						if (lineBuffer.length() == 0)
						{
							lineBuffer = null;
						}
	...
	protected String readAToken()
	{
		...
		if (ch == '"')
		{
			...
			while (ch != '"')
			{
				...
				if (ch == '\n')
				{
					new FatalError("No terminating quote for quoted string in \"" + fileName + "\"");
					// Never reaches here.
				}
			}
