...
public class Console extends JFrame implements MouseListener, MouseMotionListener, MouseWheelListener
{
	...
	/**
	 * Special code for no key (e.g. getKeyCode returns this if no key currently
	 * held down)
	 **/
	...
	// The upper-left coordinate of the drawing area seems to be (3, 29).
	// Possibly this value would be different on different OSs, which would
	// be a big problem. In the meantime, these constants are used to kludge
	// it so that getMouseX() and getMouseY() return the actual X and Y
	// position of the mouse pointer on the drawing area in Windows XP.

	private static final int mouseAdjustX = 3, mouseAdjustY = 29;

	private boolean mouseButton[] = { false, false, false };
	private int mouseX = mouseAdjustX, mouseY = mouseAdjustY, mouseClick = 0, mouseWheelRotation = 0, mouseWheelUnitsToScroll = 0;
	...
	/**
	 * @param fontSize
	 *            Default font size for println and print
	 **/
	...
	/**
	 * Use this method to copy a rectangular area of the screen to a new
	 * location
	 * 
	 * @param x
	 *            Top left corner x coordinate
	 ...
	 * @param delta_x
	 *            Will be copied at x = x + delta_x
	 * @param delta_y
	 *            Will be copied at y = y + delta_y
	 */
	...
	/**
	 * @param c
	 *            New drawing color for graphics and text
	 */
	...
	/**
	 * NOTE: This command only sets the background color. Nothing will change on
	 * the drawing surface until you use clearRect() or clear()
	 * 
	 * @param c
	...
	/**
	 * Sets XOR painting mode. In this mode, every pixel drawn to the screen
	 * will be combined with the pixel already on the screen to create an
	 * "outline" effect of what was beneath it. In this mode, if you draw the
	 * same object twice in the same location, it will appear and then
	 * disappear. See also setPaintMode().
	 * 
	 * @param c
	 *            Color to use as the background color for computation of new
	 *            color
	 */
	 ...
	 * 
	 * @param x
	 *            An array of x coordinates for the corner points of the polygon
	 *            (related to array y)
	 * @param y
	 *            An array of y coordinates for the corner points of the polygon
	 *            (related to array x)
	 * @param n
	...
	/**
	 * Draws an arc outline on the drawing area. The arc will be inscribed
	 * within a rectangle, just like drawOval(), but only part of the oval will
	 * be drawn. Draws an oval outline on the drawing area, inscribed within a
	 * rectangle.
	 * 
	 * @param x
	 *            Top left X coordinate of the rectangle
	 ...
	 * @param startAngle
	 *            Start angle of arc in degrees (0 = right middle, 90 = top
	 *            middle, etc.)
	 * @param arcAngle
	 *            Number of degrees of arc to draw
	 */
	 ...
	 * slice). The arc will be inscribed within a rectangle, just like
	 * drawOval(), but only part of the oval will be drawn. Draws an oval
	 * outline on the drawing area, inscribed within a rectangle.
	 * 
	 * @param x
	 *            Top left X coordinate of the rectangle
	 * @param y
	 ...
	 *            Height in pixels
	 * @param xRadius
	 *            Horizontal radius of arc for corners
	 * @param yRadius
	 *            Vertical radius of arc for corners
	 */
	 ...
	 *            Height in pixels
	 * @param raised
	 *            Draw rectangle as raised or sunk in
	 */
	...
	/**
	 * Draws a string on the drawing area using the current font and color. Note
	 * that the coordinates specify the bottom left corner rather than the top
	 * left corner for drawing.
	 * 
	 * @param str
	 *            The string to draw.
	 * @param x
	...
	/**
	 * Draws specified image on the drawing area. Note that if the image takes a
	 * while to load, this method will delay until it is loaded, timing out
	 * after 1000 ms.
	 * 
	 * @param img
	 *            The image to draw
	 ...
	 *            Top left y coordinate
	 * @param width
	 *            Compress or stretch image to this width
	 * @param height
	 *            Compress or stretch image to this width
	 */
	...
	/**
	 * Draws a star outline on the screen from (x, y) to (x + width, y + width).
	 * Adapted from hsa.
	 *
	 * @param x
	...
	/**
	 * Draws a maple leaf outline on the screen from (x, y) to (x + width, y +
	 ...
	 * Console followed by a newline. Adapted from hsa.
	 *
	 * @param number
	...
	/**
	 * Writes the text representation of an 8-bit integer (a "byte") to the
	 * Console with a specified field size followed by a newline. Adapted from
	 * hsa.
	 * 
	 * @param number
	 *            The number to be written to the Console.
	...
	/**
	 * Writes a character to the Console followed by a newline. Adapted from
	 * hsa.
	 *
	 * @param ch
	...
	/**
	 * Writes a character to the Console with a specified field size. Adapted
	 * from hsa.
	 *
	 * @param ch
	 *            The character to be written to the Console.
	 ...
	 * places followed by a newline. Adapted from hsa.
	 *
	 * @param number
	 *            The number to be written to the Console.
	 * @param fieldSize
	...
	/**
	 * Writes a floating point number (a "float") to the Console followed by a
	 * newline. Adapted from hsa.
	 *
	 * @param number
	 *            The number to be written to the Console.
	 */
	...
	/**
	 * Writes the text representation of an 8-bit integer (a "byte") to the
	 * Console with a specified field size. Adapted from hsa.
	 *
	 * @param number
	 *            The number to be written to the Console.
	 * @param fieldSize
	...
	/**
	 * Writes a floating point number (a "float") to the Console. Adapted from
	 * hsa.
	 *
	 * @param number
	 *            The number to be written to the Console.
	...
	/**
	 * Writes a string to the Console with a specified field size. Adapted from
	 * hsa.
	 *
	 * @param text
	...
	/**
	 * Writes the text representation of a boolean to the Console. Adapted from
	 * hsa.
	 *
	 * @param value
	...
	/**
	 * Writes a string to the Console with a specified field size followed by a
	 * newline. Adapted from hsa.
	 *
	 * @param text
	 *            The string to be written to the Console.
	...
	/**
	 * Writes the text representation of a boolean to the Console followed by a
	 * newline. Adapted from hsa.
	 *
	 * @param value
	 *            The boolean to be written to the Console.
	...
	/**
	 * Reads a boolean from the Console. The actual text in the Console must be
	 * either "true" or "false" although case is irrelevant.
	 *
	...
	/**
	 * Reads an 8-bit integer (a "byte") from the Console. The actual text in
	 * the Console must be a number from -128 to 127.
	 *
	...
	/**
	 * Reads a 32-bit integer (an "int") from the Console. The actual text in
	 * the Console must be a number from -2147483648 to 2147483647.
	 *
	...
	/**
	 * Reads a 16-bit integer (a "short") from the Console. The actual text in
	 ...
	 * @return True if the specified key is held down right now. False
	 *         otherwise.
	 */
	...
	/*
	 * Enables mouse events; included so that the overhead of listening for
	 * mouse events is not a factor if the programmer doesn't want to use the
	 * mouse.
	 * 
	 * You MUST enableMouseMethods() if you want to use the mouse!
	 */
	public void enableMouseMethods()
	{
	...
	/*
	 * Returns the rotation of the mouse wheel since the last time the wheel was
	 * queried.
	 */
	...
	/*
	 * Returns the number of units the mouse wheel has been scrolled since the
	 * last time the wheel was queried.
	 */
