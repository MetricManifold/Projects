...
public class ConsoleCanvas extends JPanel implements ActionListener, KeyListener
{
	...
	private int flashSpeed = 20; // speed is in frames (see framesPerSecond
									// above)
	...
	// ***** Keyboard Buffer & Input Variables - adapted from original hsa
	// package *****
	...
	public void print(String text)
	{
		...
		while (true)
		{
			...
			while ((index < len) && (text.charAt(index) != '\n') && (text.charAt(index) != '\t') && (index - start < maxCol - currentCol))
			{
				index++;
			}
			if (start != index)
	...
	public synchronized char readChar()
	{
		...
		while (true)
		{
			...
			} else
			{
				...
				{
					container.print(ch);
					// System.out.println(currentCol+" "+actualCol+"
					// "+cursorCol);
				}
	...
	private synchronized void invertScreen()
	{
		 ...
		 *
		 * g.translate(MARGIN, MARGIN); g.setColor(Color.white);
		 * g.setXORMode(Color.black);
		 * 
		 * // Invert the screen g.fillRect(0, 0, numXPixels, numYPixels);
		 * Toolkit.getDefaultToolkit().sync();
		 * 
		 * // Wait 50 milliseconds try { Thread.sleep(50); } catch (Exception e)
		 * { }
		 * 
		 * // Restore the screen g.fillRect(0, 0, numXPixels, numYPixels);
		 * Toolkit.getDefaultToolkit().sync();
		 * 
		 * g.setPaintMode();
		 */
