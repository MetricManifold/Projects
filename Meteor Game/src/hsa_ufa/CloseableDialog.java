 ...
 * <p>
 * The CloseableDialog class is the base class for Dialog objects created by the
 * hsa package. Dialogs based on this class will close when the dialog's close
 * box is clicked.
 * <p>
 * Full documentation for the classes in the hsa package available at: <br>
 ...
 */
class CloseableDialog extends java.awt.Dialog implements java.awt.event.ActionListener, java.awt.event.WindowListener
{
	...
	{
		super(parent == null ? new java.awt.Frame() : parent, title, true);
		setResizable(false);
	...
	/**
	 * Places the dialog in the centre of the window passed in, or in the centre
	 * of the screen if no window is provided.
	 *
	 * @param window
	 *            The window to centre the dialog on
	 */
	public void positionDialog(java.awt.Window window)
	{
		...
		if (window == null)
		{
			...
			loc = new java.awt.Point((screenSize.width - dlgSize.width) / 2, (screenSize.height - dlgSize.height) / 3);
		} else
		{
			java.awt.Dimension windowSize = window.getSize();
			java.awt.Point windowLoc = window.getLocation();
			loc = new java.awt.Point(windowLoc.x + (windowSize.width - dlgSize.width) / 2, windowLoc.y + (windowSize.height - dlgSize.height) / 3);
		}
		setLocation(loc);
	...
	/**
	 * Closes the dialog. Called by the system when the close box of the dialog
	 * has been clicked.
	 *
	 * @param e
	 *            The window event
	...
	/**
	 * Does nothing. Called by the system after the dialog has been opened for
	 * the first time.
	 *
	 * @param e
	 *            The window event
